...
/**
 * Builder class for a AccessControlPolicy object
 * version 20170908
 *
 * author: Sebastian Goendoer
 * copyright: Sebastian Goendoer <sebastian [dot] goendoer [at] gmail [dot] com>
 */
class AccessControlPolicyObjectBuilder extends ObjectBuilder
{
	...
	public static function buildFromJSON($json)
	{
		...
		return (new AccessControlPolicyObjectBuilder())
	...
	public function build()
	{
		...
		if($this->directive != AccessControlPolicyObject::DIRECTIVE_DENY 
			&& $this->directive != AccessControlPolicyObject::DIRECTIVE_ALLOW)
			throw new IllegalModelStateException('Invalid directive');
		
		if($this->entityType != AccessControlPolicyObject::ENTITY_TYPE_ALL 
			&& $this->entityType != AccessControlPolicyObject::ENTITY_TYPE_FRIENDS 
			&& $this->entityType != AccessControlPolicyObject::ENTITY_TYPE_GROUP 
			&& $this->entityType != AccessControlPolicyObject::ENTITY_TYPE_INDIVIDUAL)
			throw new IllegalModelStateException('Invalid entityType');
		
		if(($this->entityType == AccessControlPolicyObject::ENTITY_TYPE_ALL 
				|| $this->entityType == AccessControlPolicyObject::ENTITY_TYPE_FRIENDS) 
			&& $this->entityID != AccessControlPolicyObject::WILDCARD)
			throw new IllegalModelStateException('Invalid combination of entityType and entityID');
		
		if($this->entityType == AccessControlPolicyObject::ENTITY_TYPE_GROUP && !UOID::isValid($this->entityID))
			throw new IllegalModelStateException('entityType GROUP requires a valid UOID as entityID');
		
		if($this->entityType == AccessControlPolicyObject::ENTITY_TYPE_INDIVIDUAL && !GID::isValid($this->entityID))
			throw new IllegalModelStateException('entityType INDIVIDUAL requires a valid GlobalID as entityID');
		
		if($this->targetType != AccessControlPolicyObject::TARGET_TYPE_INTERFACE
            && $this->targetType != AccessControlPolicyObject::TARGET_TYPE_CONTENT)
			throw new IllegalModelStateException('Invalid targetType');
		
		if($this->target == NULL)
			throw new IllegalModelStateException('Invalid target');
		
		if($this->accessType != AccessControlPolicyObject::ACCESS_TYPE_READ
			&& $this->accessType != AccessControlPolicyObject::ACCESS_TYPE_WRITE
			&& $this->accessType != AccessControlPolicyObject::WILDCARD)
			throw new IllegalModelStateException('Invalid accessType');
		
		return new AccessControlPolicyObject($this);
	}
}